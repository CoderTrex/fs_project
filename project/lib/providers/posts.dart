...
class Posts with ChangeNotifier {
  ...
  Future<void> fetchAndSetPosts(String boardId) async {
    final filterString = 'orderBy="bo+ardId"&equalTo="$boardId"';
    var url = Uri.parse(
        'https://chatting-test-863cb-default-rtdb.asia-southeast1.firebasedatabase.app/posts.json?auth=$authToken');
    // 'https://chatting-test-863cb-default-rtdb.firebaseio.com/posts.json?auth=$authToken&$filterString');
    try {
      ...
      extractedData.forEach((postId, postData) {
        loadedPosts.add(Post(
          ...
          contents: postData['contents'].toString(),
          datetime: DateTime.parse(postData['datetime']),
          boardId: postData['boardId'].toString(),
          userId: postData['creatorId'].toString(),
        ));
      });
      _items = [];
      _items = loadedPosts;
    ...
    try {
      ...
      extractedData.forEach((postId, postData) {
        ...
        if (title.contains(SearchText) ||
            (con.contains(SearchText)) && !SearchText.isEmpty) {
          loadedPosts.add(Post(
            id: postId,
            title: postData['title'].toString(),
            contents: postData['contents'].toString(),
  ...
  Future<void> updatePost(String? id, Post newPost) async {
    ...
    if (postIndex >= 0) {
      final url = Uri.parse(
          'https://chatting-test-863cb-default-rtdb.asia-southeast1.firebasedatabase.app/posts/$id.json?auth=$authToken');
